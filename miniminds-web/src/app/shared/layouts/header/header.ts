...
export class Header implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    if (this.currentUser) {
      ...
      this.messageCountIntervalId = setInterval(() => this.loadMessagesUnreadCount(), 30000);
      if (this.isParent) {
  ...
  private clearMessageCountInterval(): void {
    if (this.messageCountIntervalId) {
      clearInterval(this.messageCountIntervalId);
      this.messageCountIntervalId = undefined;
    }
  }
  ...
  loadMessagesUnreadCount(): void {
    this.messagesService.getInbox().subscribe({
      next: (messages) => {
        this.messagesUnreadCount = messages.filter(m => !m.isRead).length;
      },
      error: () => {
